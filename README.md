...
| **api** | REST API (SDK + Admin) — ไม่จัดการ mail ingestion แล้วใน v3.0 | Go + Fiber | `4000` |
...
| `INGEST_TIMEOUT` | `30s` | Mail ingest API timeout (legacy) |
| `WORKER_CONCURRENCY` | `50` | Worker concurrent goroutines (async mail + cleanup) |
...
- mail-edge ส่งเมลผ่าน Redis queue → ไม่ต้อง HTTP call / API Key ไป API
...
> **v3.0 Note**: mail-edge ไม่ใช้ API Key ส่งเมลไป api แล้ว — ส่งตรงผ่าน Redis queue แทน
...
| `APIKey` | `api_keys` | API keys (SHA-256 hashed, permissions, managed via admin) |
